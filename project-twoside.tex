\documentclass[11pt,twoside,openright]{report}  % dimensione di default del font; quando l'opzione "draft" (brutta copia) è attiva: vengono mostrati
...
\usepackage[hcentering,bindingoffset=8mm]{geometry}       % dimensione e margini delle pagine
\usepackage{emptypage}      % evita header e footer in pagine vuote
...
%\usepackage{epstopdf}      % conversione da EPS a PDF (decommentare se la propria implementazione di pdflatex lo richiede)
\usepackage[firstpage=true]{background} % watermark in prima pagina
...
%\usepackage[normalem]{ulem}% testo rigato
...
\DeclareGraphicsExtensions{.pdf,.eps,.png,.jpg} % estensioni dei file di immagine (in ordine di preferenza)
...
\input{./tex/hyphen}        % sistemazione manuale di underfull/overfull hbox

\newcounter{includepdfpage} % counter per le pagine pdf incluse

\begin{document}            % INIZIO CONTENUTO DEL DOCUMENTO

\include{./tex/title}       % serve una pagina titolo più figah
